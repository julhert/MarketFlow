                ...
                <div class="w-full lg:w-[65%] bg-white rounded-2xl shadow-xl border border-gray-100 overflow-hidden" style="font-family: 'Julius Sans One', sans-serif;">
                    <div class="p-8 md:p-5 space-y-8">
                        @foreach($itemsCarrito as $item)
                            <div class="flex items-center justify-between border-b pb-4 mb-4">
                                <div class="flex items-center gap-4">
                                    ...
                                    @endphp
                                    @if($portada)
                                        <img src="{{ asset('storage/' . $portada->rutaImagen) }}" alt="{{ $item->producto->nombre }}" class="w-16 h-16 object-cover rounded shadow">
                                    @else
                        ...
                        <div class="flex justify-between items-center pt-4">
                            ...
                            <span class="text-2xl font-black text-orange-600">${{ number_format($totalCompra, 2) }}</span>
                        </div>
                    </div>
                </div>
                    ...
                    <div class="bg-main-dark rounded-4xl p-10 shadow-2xl text-black border border-white/5" style="font-family: 'Julius Sans One', sans-serif;">
                        ...
                        <div class="space-y-8">
                            <div class="space-y-3">
                                ...
                                <div class="relative">
                                    <select wire:model.live="id_direccion"
                                            id="id_direccion_select"
                                        ...
                                        @foreach($direcciones as $dir)
                                            <option value="{{ $dir->id_direccion }}">{{ $dir->calle }} {{ $dir->numero_exterior }} {{ $dir->colonia }}</option>
                                        @endforeach
                                    </select>
                                </div>
                            </div>
                            ...
                            <div class="space-y-4">
                                ...
                                <div class="grid gap-3">
                                    <label for="pago_debito" class="flex items-center justify-between p-4 bg-gray-50 border-2 border-white/10 rounded-2xl cursor-pointer">
                                        <div class="flex items-center gap-4">
                                            <input type="radio" id="pago_debito" wire:model="metodoPago" value="Tarjeta de Débito" name="pago" class="w-5 h-5 text-btn-buy">
                                            <span class="text-sm font-bold">Tarjeta de Débito</span>
                                        </div>
                                    </label>

                                    <label for="pago_paypal" class="flex items-center justify-between p-4 bg-gray-50 border-2 border-white/10 rounded-2xl cursor-pointer">
                                        <div class="flex items-center gap-4">
                                            <input type="radio" id="pago_paypal" wire:model="metodoPago" value="PayPal" name="pago" class="w-5 h-5 text-btn-buy">
                                            <span class="text-sm font-bold">PayPal</span>
                                        </div>
                                    </label>

                                    <label for="pago_credito" class="flex items-center justify-between p-4 bg-gray-50 border-2 border-white/10 rounded-2xl cursor-pointer">
                                        <div class="flex items-center gap-4">
                                            <input type="radio" id="pago_credito" wire:model="metodoPago" value="Tarjeta de Crédito" name="pago" class="w-5 h-5 text-btn-buy">
